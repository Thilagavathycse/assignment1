    ...
    for vehicle in vehicle_obj_list:
        if vehicle_select == vehicle.vehicle_type:
                if vehicle_select == 'auto':
                    decision=input("auto is not available without AC,do you want to choose another category(yes/no):")
                    ac_select = 'yes'
                    if decision == "no":
    ...
    for vehicle in vehicle_obj_list:
        if vehicle_select == vehicle.vehicle_type:
            if vehicle.select_ac == 'yes':
                if ac_select == 'yes':
                    print('Ac {} vehicle range:{}km price is : {}rupees/km'.format(vehicle.vehicle_type, vehicle.ac.get_range(), vehicle.ac.get_ac_price()))
            elif vehicle.select_ac == 'no':
                if ac_select == 'no':
                    print('Non-Ac {} vehicle range:{}km price is : {}rupees/km'.format(vehicle.vehicle_type, vehicle.non_ac.get_range(), vehicle.non_ac.get_non_ac_price()))
    preference=input("Do you want to book (yes/no):")
    if preference == "yes":
